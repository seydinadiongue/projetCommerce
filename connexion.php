                ...
                <div class="content">
                    ...
                    <div id="slideshow">
                        <div class="one">
                            ...
                            <p>Un produits de votre vendeur préféré</p>
                        </div>
                        <div class="two">
                            <h2><span>Vendre</span></h2>
                            <p>Devenez des vendeurs</p>
                        </div>
                        <div class="three">
                            <h2><span>Rechercher</span></h2>
                            <p>Trouvez des produits et collections qui vous plaisent</p>
                        </div>
                ...
                <div class="user">
                   ...
                   <div class="form-wrap">
                        ...
                        <div class="tabs-content">
                            ...
                            <!-- TABS CONTENT SIGNUP -->
                            <div id="signup-tab-conten">
                                <form class="signup-form" action="connexion.php" method="post" style='display : one'>
									<div class='ins'>
										<div class='reus' style='color : green; display : none'>Vous etes maintenant client de ce site</div>
										<div class='remp' style='color : maroon; display : none'>Veuillez saisir tous les champs</br></div>
										<select name='profil' class='select'>
											<option value=''>Je m'inscris comme :</option>
										...
										<input type="number" class="input" name="cni" id="cni" autocomplete="off" placeholder="Identification">
										<input type="text" class="input" name="nom" id="nom" autocomplete="off" placeholder="Nom">
										<input type="text" class="input" name="prenom" id="prenom" autocomplete="off" placeholder="Prenom">
										<input type="text" class="input" name="adresse" id="adresse" autocomplete="off" placeholder="Adresse">
										<!--    <input type="text" class="input" name="email" id="email" autocomplete="off" placeholder="Adresse mail">-->
											<input type="tel" pattern="7[0,6,7,8][0-9]{3}[0-9]{2}[0-9]{2}" class="input" name="tel" id="contact" autocomplete="off" placeholder="Contact">
											<input type="password" class="input" name="mdp" id="mdp" autocomplete="off" placeholder="Mot de passe">
											<input type="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$" class="input" name="email" id="email" autocomplete="off" placeholder="Email">
										<!--    <input type="submit" class="button" value="Valider">-->
										<p><input type="reset" style="margin-right : 40%;" class='sub btn' value="Annuler">
										<input type="submit"  class='btn btn-primary' value="S'inscrire"></p>
										
									</div>
									<!--<button class='btn prec' style='margin-left : 0%; display : none'> <- Precedant </button>
									<button class='btn suiv' style='margin-left : 80%;'> Suivant -> </button>-->
                                </form>
								<form method='post' action='connexion.php' enctype='multipart/form-data'>
									<div class='cant' style='display : none'>
										<span style='color : white'>Vous etes maintenant commercant .</span><hr/>
										<span style='color : white'>Decrivez votre espace de travail (facultatif): </span></br></br>
										<input type="text" name="namespace" class="form-control" placeholder="Nom de votre espace"/>
										</br><input type="text" name="lieuspace" class="form-control" placeholder="Lieu de l'espace"/>
										</br><span style="color : white">Photo de votre espace : </span>
										<input type='file' name='tof' class="form-control"/></br>
										<input type="submit" value="soumettre" class="btn btn-primary" style="float : right"/></br></br>
									</div>
								</form>
								<form method='post' action='connexion.php'>
									<div class='choix' style='display : none'>
										<span style='color : white'>Il vous reste à definir des collections que vous auriez dans votre espace </br><hr/> Nombre max de choix : 5</span></br></br>
										<div  style='display : inline-block'>
											<div style='width : 100% ; color : white'>Vetement : <input style='float : right' type='checkbox' name='col[]' value='vetement'></div>
											<div style='width : 100% ; color : white'>Chaussure : <input  style='float : right'type='checkbox' name='col[]' value='chaussure'></div>
											<div style='width : 100% ; color : white'>Sac : <input style='float : right'type='checkbox' name='col[]' value='sac'></div>
											<div style='width : 100% ; color : white'>Ordinateur : <input style='float : right'type='checkbox' name='col[]' value='ordinateur'></div>
											<div style='width : 100% ; color : white'>Telephone : <input style='float : right'type='checkbox' name='col[]' value='telephone'></div>
										</div>
										<div  style='display : inline-block; float : right'>
											<div style='width : 100% ; color : white'>Montre : <input style='float : right'type='checkbox' name='col[]' value='montre'></div>
											<div style='width : 100% ; color : white'>Refrigerateur : <input style='float : right'type='checkbox' name='col[]' value='refrigerateur'></div>
											<div style='width : 100% ; color : white'>Lunette : <input style='float : right'type='checkbox' name='col[]' value='lunette'></div>
											<div style='width : 100% ; color : white'>Table : <input style='float : right'type='checkbox' name='col[]' value='table'></div>
											<div style='width : 100% ; color : white'>Feuteuil : <input style='float : right'type='checkbox' name='col[]' value='feuteuil'></div>
										</div></br></br>
										<p><a href='index.php' style="margin-right : 45%;" class='sub btn btn-default'><span style='color : black'>Ignorer</span></a>
										<input type="submit"  class='btn btn-primary' value="Ajouter"></p>
									</div>
								</form>
                            </div>
							<?php
							$dsn = 'mysql:host=localhost;dbname=base';
							$bdd= new PDO($dsn, 'root', '');
							$requete = 'insert into collection (nomcol, idcom) values (:nomcol, :idcom)';
							$st = $bdd->prepare($requete);
							if(isset($_POST['cni']) and isset($_POST['nom']) and isset($_POST['prenom']) and isset($_POST['mdp']) and isset($_POST['adresse']) and isset($_POST['tel']) and isset($_POST['email'])){
								$profil = $_POST['profil'];
								$cni = $_POST['cni'];
								$nom = $_POST['nom'];
								$prenom = $_POST['prenom'];
								...
								$tel = $_POST['tel'];
								$email = $_POST['email'];
								if(!empty($cni) and !empty($nom) and !empty($prenom) and !empty($mdp) and !empty($adresse) and !empty($tel) and !empty($email)  ){
									if($_POST['profil']=='client')
										$req='INSERT INTO client (`idclient`,`nom`, `prenom`, `mdp`, `adresse`, `tel`, `email`) VALUES (:idpers, :nom, :prenom,:mdp,:adresse,:tel,:email)';
									else if($_POST['profil']=='vendeurtable')
										$req='INSERT INTO vendeurtable (`idvend`,`nom`, `prenom`, `mdp`, `adresse`, `tel`, `email`) VALUES (:idpers, :nom, :prenom,:mdp,:adresse,:tel,:email)';
									else if($_POST['profil']=='commercant')
										$req='INSERT INTO commercant (`idcom`,`nom`, `prenom`, `mdp`, `adresse`, `tel`, `email`) VALUES (:idpers, :nom, :prenom,:mdp,:adresse,:tel,:email)';
									else
										$req='INSERT INTO marchand (`idmar`,`nom`, `prenom`, `mdp`, `adresse`, `tel`, `email`) VALUES (:idpers, :nom, :prenom,:mdp,:adresse,:tel,:email)';
									$stmt=$bdd->prepare($req);
									$stmt->bindParam(':idpers', $cni);
									...
									if($stmt->execute()){
										if($_POST['profil']!='client' && $_POST['profil']!='marchand'){
											$_SESSION['idpers'] = $cni;
											$_SESSION['profil'] = $_POST['profil'];
											echo "
											<script>
										
												$('.ins').slideUp(100,'linear');
												$('.cant').slideDown(1200,'linear');
												
											</script>";
										}else{
											echo "
											<script>
												alert('Inscription reussie!! Vous etes maintenant client de ce site');
											</script>";
										}
									}
								}else{
							...
							if(isset($_POST['col'])){
								if(count($_POST['col'])<6){
									$cols = $_POST['col'];
									foreach($cols as $col){
										$st->bindParam(':nomcol', $col);
										$st->bindParam(':idcom', $_SESSION['idpers']);
										if($st->execute()){
											echo "<script>alert('Inscription reussie à 100% !!')</script>";
										}
									}
								}
								error_reporting(0);
							...
							if(isset($_POST['namespace'])){
								...
								if($_SESSION['profil']=='commercant')
									$re = 'insert into cantine (nomcant, lieu, idcom,photo) values (:namespace, :lieu,:idpers,:photo)';
								else
									$re = 'insert into tablee (nomtable, lieu, idcom) values (:namespace, :lieu,:idpers)';
								$stmt = $bdd->prepare($re);
								$url = faire();
								$stmt->bindParam(':namespace', $_POST['namespace']);
								$stmt->bindParam(':lieu', $_POST['lieuspace']);
								$stmt->bindParam(':idpers', $_SESSION['idpers']);
