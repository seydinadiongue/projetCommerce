...
$req = "select * from marchand where conf='oui'";
$stmt = $bdd->query($req);
...
while($row = $stmt->fetch()){
	...
	<tr class="something">
		...
		if($row['etat']=='normal')
			echo '<td style="text-align : center;"><button data-id="'.$row['idmar'].'" class="bloquer_mar btn btn-default btn-xs">Bloquer</button></td>';
		else
			echo '<td style="text-align : center;"><button href="#" data-id="'.$row['idmar'].'" class="bloquer_mar btn btn-danger btn-xs">Compte bloqué</button></td>
	</tr>
	';
}
echo'</tbody>';
