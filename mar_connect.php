...
<div id="wapper" >  <!------ wrapper permet d'etirer l'header---------->
 ...
 <nav class="navbar navbar-expand-lg navbar-inverse navbar-fixed-top" style="height : 1px ">
  ...
  <div style='background : white; margin : 0 13% 0 13%; padding-bottom : 10%; '>

   <?php
		$idcom = $_GET['idcom'];
		$bdd= new PDO("mysql:host=localhost;dbname=base", 'root', '');
		$stmt = $bdd->query('select * from marchand where idmar='.$idcom);
		$row = $stmt->fetch();
	?>
	
	<?php
        echo '<img style="margin-left : 35%;margin-top : 5%;width : 300px;height: 300px;  border-radius : 20%; border : 5px solid white" src="'.$row['pp'].'"/>';
		echo '</br><div style="font-weight : bold; font-size : 25px" class="card-body text-center">'.$row['prenom'].' '.$row['nom'].'</div>';
		echo '
			<form method="post" action="mar_connect.php?idcom='.$_GET['idcom'].'">
				<input type="text" name="mess" style="height : 75px;width : 60%; margin-left : 20%" placeholder="L\'envoyer un message" class="form-control">
				<input type="submit" name="submit" value="Envoyer" style="display : none"/>
			</form>
		';
		$row = $bdd->query('select * from commercant where idcom = '.$row['idmar'])->fetch();
