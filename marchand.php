...
<h2 class="text-center">Binevenue dans la foule marchands ambulants</h2><hr/>
<?php
$dsn = 'mysql:host=localhost;dbname=base';
$bdd= new PDO($dsn, 'root', '');
$req = 'Select * from produit p JOIN image i JOIN marchand m on p.idprod=i.idprod and p.idpers=m.idmar where p.idprod not in (select idprod from reserver) group by p.idprod ';
$urls = $bdd->query($req);
$t=0;
		...
		<div class="col-md-3" style="margin-bottom : 4%;">
				<div class="" style="width: 225px;margin-bottom : %; border-radius : 0%; border : 1px black solid; background : white">
					<div style="display : inline-block; float : left ;  margin : 2% 5% 0 -10%">
						<a href="lemarchand.php?idcom='.$url['idmar'].'"><img style=" width : 35px; height : 35px; margin-left : 100%;border-radius : 50%" src="'.$url['pp'].'"/></a>
					</div>
					...
					<div class="inner">
						<img style=" margin-top : 2%;width : 200px; height : 150px;margin-left : 5%; margin-right : 5%; border-radius : 10%;" src="'.$url['url'].'" class="card-img-top " alt="..."><hr/>
					</div>
				<div class="text-center" style="height : 3rem; margin-top : -5% " >
					<a href = "acheter.php" class="btn btn-primary btn-xs">Acheter</a>
					<a href="marchand.php?idprod='.$url['idprod'].'"><input type="submit" value = "Reserver" class="btn btn-primary btn-xs" onclick="document.getElementById(\'modal-wrapper\').style.display=\'block\'"/></a>
					<a href = "detailprodmar.php?idprod='.$url['idprod'].'&nomprod='.$url['nomprod'].'&idmar='.$url['idmar'].'" class="btn btn-default btn-xs">Details</a>
				</div>
			</div>
		</div>
	';
}
if($t==0) echo '<div class="text-center" style="margin-top : 10%; font-size : 150%">Aucun produit en ligne</div>';
error_reporting(0);
?>
<?php 
...
<form class="col-lg-6 animate" method="post" action="marchand.php?idprod='.$_GET["idprod"].'" style="background-color : rgba(169, 169, 169, 0.7);box-shadow : 5px black; border-radius : %;padding-bottom : 3% ">
...
<?php
	...
	if(isset($_POST['login']) and isset($_POST['mdp'])){
		...
		if(!empty($login) and !empty($mdp)){
			...
			$res = $st->fetch();
			if($res){
				$stmt = $bdd->exec('INSERT INTO `reserver`(`idprod`, `idclient`) VALUES ('.$_GET['idprod'].', '.$res['idclient'].')');
				if($stmt)
