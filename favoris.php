...
<?php
	...
	$req = 'Select * from produit p JOIN image i JOIN favoris f on p.idprod=i.idprod and i.idprod=f.idprod where f.idclient = '.$_SESSION['idpers'].' group by p.idprod';
	$stmt = $bdd->query($req);
	echo '<table class="table" style=" margin : auto; margin-top : 7%; width : 70%">
		...
		while($row = $stmt->fetch()){
			$supp = 'favoris.php?idprod='.$row['idprod'].'&act=supp';
			$lign = $bdd->query("select * from cantine where idcom=".$row['idpers'])->fetch();
			$mod = 'detailprod.php?idprod='.$row['idprod'].'&nomprod='.$row['nomprod'].'&cant='.$lign['nomcant'];
			echo'
			<tr class="something">
				...
				<td style="text-align : center;"><button class="btn btn-default test" onclick="$(\'#confirmModal\').modal(\'show\')">Annuler reservation</button></td>
				...
				<div class="modal-dialg">
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<h5 class="text-center">Voulez-vous vraiment annuler votre reservation?</h5><hr/>
				...
				<td style="text-align : center;"><a href='.$mod.' class="btn btn-default">Details</a></td>
			</tr>
			';
		}
	echo'</tbody>
...
<?php
	$dsn = 'mysql:host=localhost;dbname=base';
	$bdd= new PDO($dsn, 'root', '');
	if(isset($_GET['idprod']) and isset($_GET['act']) and $_GET['act']=="supp"){
		if($_GET['act']=='supp'){
			$lign = $bdd->query("select idpers from produit where idprod=".$_GET["idprod"])->fetch();
			$res =$bdd->exec("delete from reserver where idprod=".$_GET['idprod']);
			$res =$bdd->exec("delete from favoris where idprod=".$_GET['idprod']);
			$res =$bdd->exec("insert into notif_client (idcom, idclient, idprod, action, date) values (".$lign['idpers'].", ".$_SESSION['idpers'].", ".$_GET['idprod'].", 'annulle_reserv_client', now())");
			if($res)
				echo '<script>alert("Vous avez annule la reservation !!!!")</script>';
