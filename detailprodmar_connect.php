...
<?php
	if(isset($_GET['nomprod'])){
		...
		if (isset($_GET['idmar'])){
			echo '<h2 class="text-center">Bienvenue dans la foule des marchands ambulants</h2><hr/>';
			$row = $bdd->query('select * from marchand where idmar='.$_GET['idmar'])->fetch();
		}
		else{
			echo '<h2 class="text-center">Bienvenue dans la foule des vendeurs de table</h2><hr/>';
...
<div style=" margin-right : %; margin-bottom : 10%" class="col-lg-7">
