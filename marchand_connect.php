...
<div id="wapper" >  <!------ wrapper permet d'etirer l'header---------->
	...
	<div class="container-fluid register" style="padding-top : 5%;border-top : 5px solid black;">
		...
		<div id="modal-wrapper" class="modal row" style=";margin : 4.3% 0% 0% 0%; width : 100%; height : 90%; background :  ; border : px solid black;">
			...
			<?php
				echo '<div class="text-center fixed" style="">
					<a href="cantines_connect.php"><input class="btn btn-info" style="font-weight : bold;float : left;margin : 2% 0 0 5% " type="submit" name="espaceClient" value="Cantes" /> </a>
					<a href="index.php"><input class="btn btn-info" style="font-weight : bold;  margin : 2% 0 0 0" type="submit" name="accueil" value="Marchand ambulant"/></a>
					<a href="editer.php" ><input class="btn btn-info" style="font-weight : bold; float : right;margin : 2% 5% 0  "  type="submit" name="supprimer" value="Vendeur de table"/></a>
				</div><hr/>';
			?>
				<?php
				$bdd = new PDO('mysql:host=localhost; dbname=base', 'root', '');
				...
				while($row = $stmt->fetch()){
					...
					echo '<a href="marchand_connect.php?id='.$row['envoyeur'].'" style="color : white"><div class="row">';
		...
		?>
			<?php
				echo '<div class="text-center fixed" style="">
					<a href="client.php"><input class="btn btn-default" style="font-weight : bold;float : left;margin : 2% 0 0 5% " type="submit" name="espaceClient" value="Cantines" /> </a>
					<a href="marchand_connect.php"><input class="btn btn-info" style="font-weight : bold;  margin : 2% 0 0 0" type="submit" name="accueil" value="Marchand ambulant"/></a>
			...
			?>
				<!--#########################Affichage des produits des marchands############################################-->
				<div class="form-group col-lg-7" style="margin : 1% 0 2% 25%">
			...
			<?php
				...
				$req = 'Select * from produit p JOIN image i JOIN marchand m on p.idprod=i.idprod and p.idpers=m.idmar group by p.idprod ';
				$urls = $bdd->query($req);
				echo '<div class="tout" style="margin-left : 8%">';
						...
						<div class="col-md-3" style="margin-bottom : 4%;">
								<div class="" style="width: 200px;margin-bottom : %; border-radius : 0%; border : 1px black solid; background : white">
									<div style="display : inline-block; float : left ;  margin : 2% 5% 0 -10%">
										<a href="mar_connect.php?idcom='.$url['idmar'].'"><img style=" width : 35px; height : 35px; margin-left : 100%;border-radius : 50%" src="'.$url['pp'].'"/></a>
									</div>
								...
								<div class="text-center" style="height : 3rem; margin-top : -5% " >
									...
									<a href="marchand_connect.php?idprod='.$url['idprod'].'&action=reserv"><input type="submit" value = "Reserver" class="btn btn-primary btn-xs" /></a>
									<a href = "detailprodmar_connect.php?idprod='.$url['idprod'].'&nomprod='.$url['nomprod'].'&idmar='.$url['idmar'].'" class="btn btn-default btn-xs">Details</a>
								</div>
							</div>
						</div>
					';
				...
				if(isset($_GET['idprod']) and isset($_GET['action'])){
					...
					$action=$_GET['action'];
					if($action=="reserv"){
						$res = $bdd->exec("insert into reserver (idprod, idclient, date) values (".$_GET['idprod'].", ".$_SESSION['idpers'].", now())");
						if($res) echo'<script>alert("Votre demande est envoyée avec succés !!")</script>';
						else echo'<script>alert("Echec de l\'envoie !!")</script>';
					}
				}
				//##########################################################"#################################################
				error_reporting(0);
...
<script>
		...
		$(document).ready(function(){
			...
			$('#search').keyup(function(){
				...
				if(txt!= ""){
					$.ajax({
						url:"fetch_connect_marchand.php",
						method:"post",
						data:{search:txt},
						dataType:"text",
