 ...
 <nav class="navbar navbar-expand-lg navbar-inverse navbar-fixed-top" style="height : 1px ">
  ...
  <div style='background : white; margin : 0 13% 0 13%; padding-bottom : 35%;'>
   ...
   <?php
		...
		$stmt = $bdd->query('select * from cantine ca JOIN commercant co on ca.idcom = co.idcom where co.idcom='.$idcom);
		$row = $stmt->fetch();
	?>
	
		<div style='margin-left : 25%;margin-top : 6%;margin-bottom : 15%;width : 600px;height : 300px;background : url("<?php echo $row['photo'] ?>"); background-repeat : no-repeat;background-size : 100% 100%;border-radius : 2%  '>
	<?php
        echo '<img style="margin-left : 34%;margin-top : 32%;width : 200px;height: 200px;  border-radius : 50%; border : 5px solid white" src="'.$row['pp'].'"/>';
		echo '</br><div style="font-weight : bold; font-size : 25px" class="card-body text-center">'.$row['prenom'].' '.$row['nom'].'</div>';
		echo '
			<form method="post" action="proprietaire_connect.php?idcom='.$_GET['idcom'].'">
				<input type="text" name="mess" style="height : 75px" placeholder="L\'envoyer un message" class="form-control">
				<input type="submit" name="submit" value="Envoyer" style="display : none"/>
			</form>
		';
		$row = $bdd->query('select * from commercant where idcom = '.$row['idcom'])->fetch();
  ...
  if(isset($_POST['mess'])){
	$mess=$_POST['mess'];
	if(!empty($mess)){
		$res = $bdd->exec('insert into chat(envoyeur, receveur, message, date) values ('.$_SESSION['idpers'].', '.$_GET['idcom'].', "'.$mess.'", now())');
		if($res) echo'<script>alert("Envoie reussi")</script>';
		else echo'<script>alert("Echec")</script>';
	}else echo'<script>alert("Veuillez ecrire quelques choses")</script>';
  }
	?>
	</div>
<!-- Footer -->
